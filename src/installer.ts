...
export async function getSentinel(version: string) {
  // check cache
  let toolPath: string;
  toolPath = tc.find('sentinel', version);
  ...
  if (!toolPath) {
    ...
    toolPath = await acquireSentinel(version);
...
async function acquireSentinel(version: string): Promise<string> {
  //
  ...
  //
  return await tc.cacheDir(extPath, 'sentinel', version);
}

//
// Sentinel versions interface
// see https://releases.hashicorp.com/sentinel/index.json
//
interface Build {
  name: string;
  version: string;
...
async function queryLatestMatch(versionSpec: string): Promise<string> {
  let versions: string[] = [];
  let rest: restm.RestClient = new restm.RestClient('setup-sentinel');
  let release: Release | null = (
    await rest.get<Release>(
      'https://releases.hashicorp.com/sentinel/index.json'
    )
  ).result;
...
function getDownloadUrl(version: string): string {
  return util.format(
    `https://releases.hashicorp.com/sentinel/%s/sentinel_%s_%s_%s.zip`,
    version,
    version,
    osPlat,
    osArch
