...
export async function getSentinel(versionSpec: string): Promise<void> {
  let toolPath: string
  ...
  if (!toolPath) {
    ...
    if (!version) {
      throw new Error(
        `Unable to find Sentinel version '${versionSpec}' for platform ${osPlat} and architecture ${osArch}.`
      )
    }
